...
import { set_interval, set_duration } from '../Stores/SettingsStore';
...
class RobotProxy {
    ...
    download_interval(interval) {
        BleService.sendCommandToActDevice('I' + interval);
    }
    ...
    handleResponse(responseHandler, response) {
        ...
        if (response.match("\\b[0-9]{3}\\b,\\b[0-9]{3}\\b")) {
            ...
            if (response.match("i=[0-9]{2}")) {
                let i = response.trim().split("=")
                console.log("interval: " + i[1] )
                let interval = i[1]
                responseHandler({type: "interval", i: interval})
            } else {           
                switch (response) {     
                    case (',,,,'):
