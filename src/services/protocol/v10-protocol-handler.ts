 ...
 *
 * Handles binary protocol with chunked uploads for firmware version 10.
...
export class V10ProtocolHandler implements IProtocolHandler {
  readonly version: ProtocolVersion = 'V10';
  readonly maxInstructions: number = 4096;
  ...
  async uploadProgram(
    ...
    const chunks = this.createChunks(program.instructions);

    for (let i = 0; i < chunks.length; i++) {
      await this.hardware.write(chunks[i]);
  ...
  private createChunks(instructions: RobotInstruction[]): Uint8Array[] {
